...
export default function RoleEdit() {
  ...
  useEffect(() => {
    Promise.all([
      api.get(`/roles/${id}`),
      api.get('/privilege-groups'),
    ]).then(([roleRes, groupsRes]) => {
      const r = roleRes.data;
      setForm({ name: r.name, slug: r.slug, description: r.description || '', is_active: r.is_active });
      setSelectedPrivileges(r.privileges?.map(p => p.id) || []);
      setPrivileges(groupsRes.data);
      setFetching(false);
    });
  }, [id]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.post(`/roles/${id}/assign-privileges`, { privilege_ids: selectedPrivileges });
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-8">
        <button type="button" onClick={() => navigate(`${PORTAL}/roles`)} className="p-2 -ml-2 rounded-xl hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
        </button>
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Edit Role</h1>
          <p className="text-sm text-gray-500 mt-0.5">Update role details and privilege assignments</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-3">
          ...
          <button type="button" onClick={() => navigate(`${PORTAL}/roles`)}
